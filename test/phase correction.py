...
def h_lab1(a, f, t, p):  # ideal lab frame Hamiltonian
    b = esr(a, f, t, p)
    return 1/2 * 2*np.pi * np.array([[0, 0, 0, 0],
                                     [0, dEz_tilde-J, 0, 0],
                                     [0, 0, -dEz_tilde-J, b],
                                     [0, 0, np.conj(b), 0]])
...
for t in time_list:
    h1 = h_lab1(Omega, f_2u, t, 0)
    m_ideal = np.dot(expm(-1j * 1 * h1 * dt), m_ideal)

    h2 = h_lab2(Omega, f_2u, t, 0)
...
# m_rwa1 = R2 @ m_rwa1
mod2_1u = np.diag([np.exp(1j*(dd[0][3])*T_pi_2), np.exp(1j*dd[1][3]*T_pi_2), np.exp(1j*dd[2][3]*T_pi_2), np.exp(1j*(dd[3][3])*T_pi_2)])
# m_rwa1_ = mod2_1u @ m_rwa1
m_lab_ = mod2_1u @ m_lab

# print(modulation)
# print(mod2_1u)
R3 = np.diag([1, np.exp(-1j*2*np.pi*f_1d*2*T_pi_2), np.exp(1j*2*np.pi*f_1u*2*T_pi_2), 1])
# print(m_rwa2)
# m_rwa2 = R3 @ m_rwa2
...
with np.printoptions(precision=5, suppress=True):
    ...
    # print(gate_fidelity(m_ideal, m_lab__))
